...
@section('content')
    <div class="card">
        <div class="card-header bg-warning text-dark">
            <h5 class="mb-0">Edit Ticket Service</h5>
        </div>
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.ticket_services.update', $service->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select id="main_category_id" class="form-select" required>
                        ...
                        @foreach($mainCategories as $main)
                            <option value="{{ $main->id }}" @if(optional($service->subcategory->mainCategory)->id == $main->id)
                            selected @endif>
                                {{ $main->name }}
                            </option>
                        @endforeach
