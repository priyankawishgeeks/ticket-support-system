...
@section('page_title', 'Edit App User')

@section('content')
    <div class="container mt-4">
        <div class="card shadow">
            <div class="card-header bg-warning text-white">
                <h5>Edit App User - {{ $user->user }}</h5>
            </div>
            ...
            <div class="card-body">
                <form action="{{ route('admin.app_user.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="panel" class="form-select" required>
                                ...
                                <option value="A" {{ old('panel', $user->panel) == 'A' ? 'selected' : '' }}>Admin</option>
                                <option value="S" {{ old('panel', $user->panel) == 'S' ? 'selected' : '' }}>Staff</option>
                                <option value="U" {{ old('panel', $user->panel) == 'U' ? 'selected' : '' }}>User</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            @if($user->img_url)
                                <div class="mt-2">
                                    <img src="{{ asset($user->img_url) }}" alt="Profile Image" class="img-thumbnail"
                                        width="100">
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="is_enable_chat" class="form-select">
                                <option value="1" {{ old('is_enable_chat', $user->is_enable_chat) == 1 ? 'selected' : '' }}>
                                    Yes</option>
                                <option value="0" {{ old('is_enable_chat', $user->is_enable_chat) == 0 ? 'selected' : '' }}>No
                                </option>
                            </select>
                        </div>
                    </div>
