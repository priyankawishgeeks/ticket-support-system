            ...
            <div class="col-lg-8 mb-4">
                <div class="card">
                    ...
                    <div class="card-body py-0">
                        <!-- Ticket Title & Body -->
                        <h5 class="fw-bold m-4">{{ $ticket->title }}</h5>
                        <p class="m-4">{{ $ticket->ticket_body }}</p>
                        <div class=" shadow-sm  ">
                            <!-- 🎟️ Ticket Header -->
                        ...
                        <div class="mt-4 border p-3 rounded bg-light">
                            ...
                            <div class="d-flex align-items-center justify-content-between mt-3">
                                ...
                                {{-- Priority --}}
                                <div class="d-flex align-items-center w-50 me-3">
                                    <label for="priority" class="fw-bold mb-0 me-2">Priority:</label>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function loadCannedMessages() {
                const url = new URL("{{ route('agent.canned_messages.fetch') }}", window.location.origin);
    ...
    <script>
        $(document).ready(function () {
            ...
            // 🟢 Append single reply (used by both loadReplies + new submit)
            function appendReply(reply) {
                const isStaff = reply.created_by_type === 'app_user';
                const senderName = isStaff
                    ? (reply.app_user?.name ?? 'Staff')
                    : (reply.site_user?.name ?? 'Client');
                ...
                if (reply.attachments && reply.attachments.length > 0) {
                    // If attachments are plain strings (paths)
                    attachmentsHTML = `
            ...
            $('#reply_submit_btn').on('click', function (e) {
                ...
                if (!replyBody) return alert('Please enter your reply first.');
